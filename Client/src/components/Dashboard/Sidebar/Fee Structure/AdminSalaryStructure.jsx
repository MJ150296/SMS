...
import { createAdminSalaryStructure } from "../../../../Redux/slices/adminSalarySlice.js";

const { Option } = Select;

const AdminSalaryStructureForm = () => {
  const dispatch = useDispatch();

  const { control, handleSubmit } = useForm();

  const onSubmit = (data) => {
    console.log("Admin Salary Structure Submitted:", data);
    dispatch(createAdminSalaryStructure(data));
  };
  ...
  return (
    <Form layout="vertical" onFinish={handleSubmit(onSubmit)}>
      ...
      <Form.Item label="Communication Allowance" name="communicationAllowance">
        <Controller
          name="allowances.communicationAllowance"
          control={control}
          render={({ field }) => (
            <InputNumber
              {...field}
              min={0}
              placeholder="Enter Communication Allowance"
              className="w-full"
            />
          )}
        />
...
export default AdminSalaryStructureForm;
